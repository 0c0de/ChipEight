...
void Chip8::emulateCPUCycles() {
	...
	switch (opcodes & 0xF000)
	{
		...
		case(0x9000):
			...
			//Format 0x9XY0
			if (V[(opcodes & 0x0f) >> 8] == V[(opcodes & 0x00f) >> 4]) {
				pc += 4;
			}
			else {
				pc += 2;
		...
		case(0xF000):
			switch (opcodes & 0x00ff) {
				...
				//Format 0xfX07
				case 0x65:
					for (auto q = 0; q <= (opcodes & 0x0f); q++) {
						cout << "Register V[" << q << "] and Index is I " << I + q << endl;
						V[q] = memory[I + q];
					}
					//I = ((opcodes & 0x0f) >> 8) + 1;
					pc += 2;
